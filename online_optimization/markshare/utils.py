...
def readmpsmodel(sample_name):
    ...
    c = []
    c_center = np.empty(num_c)
    for i in range(num_c):
        #c.append(cp.Parameter(nonneg=True,name='c'+str(i)))
        c.append(cp.Parameter(name='c' + str(i)))
    k = 0
    constraints = []
    for i in range(num_constraints):
        ...
        if constraint_senses[i] == 'E':
            if k < num_c:
                if match:
                    ...
                    c[k].value = constraint_rhs[i]
                    c_center[k] = constraint_rhs[i]
                    constraints.append(arr_sparse @ x == c[k])
                    k = k + 1
                else:
    ...
    objective_linear = problem.objective.get_linear()
    constant_coeffs = np.array(problem.objective.get_linear(), dtype=float)
    parameter_indices = range(0, 4)

    for i in parameter_indices:
        constant_coeffs[i] = 0
        c[k].value = objective_linear[i]
        c_center[k] = objective_linear[i]
        k += 1

    objective_expression = cp.matmul(constant_coeffs, x) + sum(c[k] * x[i] for k, i in enumerate(parameter_indices))
    object_function = cp.Minimize(objective_expression + problem.objective.get_offset())
    ...
    return cvxpy_problem, c_center


def sample_around_points(c, r,
                         n_total=8000):
    """
    input:
        b:ndarray
    ...
    np.random.seed(0)
    n = len(c)
    column_names = [f'c{i}' for i in range(len(c))]  # Generate column names like b0, b1, ..., b9
...
def get_instance(sample_name, n_instances, r, ptype='beasley'):
    mode = "train"
    ...
    parameter_indices = range(0, 4)
    original_coeffs = model.getAttr("Obj")

    original_params = [original_coeffs[i] for i in parameter_indices]
    ndim = len(parameter_indices)
    model_sense = gp.GRB.MINIMIZE if model.getAttr("ModelSense") == 1 else gp.GRB.MAXIMIZE
    if mode == "test":
        new_params_list = read_new_para(f"/home/ljj/project/mlopt/online_optimization/{ptype}/{ptype}_2_test_data.pkl")
    elif mode == "train":
        new_params_list = read_new_para(f"/home/ljj/project/mlopt/online_optimization/{ptype}/{ptype}_2_data.pkl")
    n_instances = len(new_params_list)
    def modify_objective(model, new_coeffs, model_sense):
        new_obj = original_coeffs[:]  # 拷贝原始系数
    ...
    for i in tqdm(range(n_instances)):
        ...
        try:
            ...
            print(f"Error saving instance {i}: {str(err)}")
...
filedir = '/home/ljj/project/mlopt/online_optimization/markshare'
file = filedir + '/markshare_4_0.mps'
get_instance(file, 100, r=2, ptype='markshare')
