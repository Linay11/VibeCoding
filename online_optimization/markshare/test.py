...
def test_solve():
    ...
    sol_file = filedir + '/markshare_4_0.sol'
    isfile = os.path.isfile(file)
    print(isfile)
    m = gp.read(file)
    m.optimize()
    m.reset()
    problem, c = u.readmpsmodel(file)
    column_names = [f'c{i}' for i in range(len(c))]
    df = pd.DataFrame([c], columns=column_names)
    ml = mlopt.Optimizer(problem, parallel=False)
    res = ml._problem.solve_parametric(df, parallel=True)
    print(res[0]['cost'])


def test_setcover():
    STORAGE_DIR = "/home/ljj/project/mlopt/results/markshare/setcover"
    mlopt_model_DIR = "/home/ljj/project/mlopt/results/markshare/setcover"
    EXAMPLE_NAME = STORAGE_DIR + '/markshare_'

    # Load model
    n_best = 20
    m_mlopt = mlopt.Optimizer.from_file(mlopt_model_DIR + '/markshare_model')
