...
def sample_around_points(df,
    ...
    for idx, row in df.iterrows():
        ...
        df_samples = pd.concat([df_samples, df_row])

    return df_samples


def save_multiple_lp_files(sim_data, problem, dir, base_filename):
    for i in range(len(sim_data['sol'])):
        params = {
            'E_init': sim_data['E'][i],
            'z_init': sim_data['z'][i],
            's_init': sim_data['s'][i],
            'past_d': sim_data['past_d'][i],
            'P_load': sim_data['P_load'][i]
        }
        for param, value in params.items():
            problem.param_dict[param].value = value
        filename = dir + base_filename % i
        problem.solve(solver=cp.GUROBI)
        gurobi_model = problem_to_gurobi(problem)
        gurobi_model.write(filename)


def problem_to_gurobi(problem):
    gurobi_model = gp.Model()

    # Create variables in the Gurobi model
    variables = {}
    for var in problem.variables():
        for i in range(var.size):
            lb = var.attributes.get("lb", -gp.GRB.INFINITY)
            ub = var.attributes.get("ub", gp.GRB.INFINITY)
            name = var.name() + f'[{i}]'
            if var.attributes['integer'] == True:
                variables[name] = gurobi_model.addVar(lb=lb, ub=ub, name=name, vtype=gp.GRB.INTEGER)
            else:
                variables[name] = gurobi_model.addVar(lb=lb, ub=ub, name=name)
    for i in range(5):
        name = 'P_load' + f'[{i}]'
        variables[name] = gurobi_model.addVar(name=name)


    gurobi_model.update()

    def convert_expression_to_gurobi(expr):
        # return List
        if isinstance(expr, cp.Expression):
            expr1 = expr.args[0] if len(expr.args) != 0 else expr
            aff_list = []

            if len(expr.args) == 1 or len(expr.args) == 0:
                if isinstance(expr1, cp.Constant) or isinstance(expr1, cp.Parameter):
                    if isinstance(expr1.value, float):
                        expr1_val_list = [expr1.value]
                    if expr1.value.ndim == 0:
                        expr1_val_list = [expr1.value.tolist()]
                    else:
                        expr1_val_list = expr1.value
                    if len(expr1_val_list) == 1:
                        return [expr1.value.item()]  # constant
                    else:
                        return expr1.value.tolist()
                elif isinstance(expr1, cp.Variable):
                    for idx in range(expr1.size):
                        varname = expr1.name() + f'[{idx}]'
                        v = variables[varname]
                        aff_list.append(v)
                    return aff_list

            if isinstance(expr1, cp.Expression):
                expr1 = convert_expression_to_gurobi(expr1)

            if len(expr.args) <= 1:
                return expr1

            expr2 = expr.args[1]
            if isinstance(expr2, cp.Expression):
                expr2 = convert_expression_to_gurobi(expr2)
            con = expr2
            n = len(expr1) if len(expr1) != 1 else len(con) if len(con) != 1 else 1
            for idx in range(n):
                v = expr1[idx] if len(expr1) != 1 else expr1[0]
                c = con[idx] if len(con) != 1 else con[0]
                aff = v * c
                aff_list.append(aff)
    ...
    for constr in problem.constraints:
        lhs = convert_expression_to_gurobi(constr.args[0])
        rhs = convert_expression_to_gurobi(constr.args[1])
        n = len(rhs) if len(rhs) != 1 else len(lhs) if len(lhs) != 1 else 1
        if n == 1:
            gurobi_model.addConstr(lhs <= rhs, name=constr.name()) \
                if isinstance(constr, cp.constraints.nonpos.Inequality) \
                else gurobi_model.addConstr(lhs == rhs, name=constr.name())
            continue
        else:
            for j in range(n):
                r = rhs[j] if len(rhs) != 1 else rhs[0]
                l = lhs[j] if len(lhs) != 1 else lhs[0]
                constr_name = constr.name() + f'_{j}'
                gurobi_model.addConstr(l <= r, name=constr_name) \
                    if isinstance(constr, cp.constraints.nonpos.Inequality) \
                    else gurobi_model.addConstr(l == r, name=constr_name)
    # Set objective
    objective = 0
    expr = problem.objective.expr
    if isinstance(problem.objective, cp.Minimize):
        objective = sum(
            expr[i] * variables[var.name()]
            for i, var in enumerate(expr.variables())
        )
        gurobi_model.setObjective(objective, gp.GRB.MINIMIZE)
    else:
        objective = sum(
            expr[i] * variables[var.name()]
            for i, var in enumerate(expr.variables())
        )
        gurobi_model.setObjective(objective, gp.GRB.MAXIMIZE)

    return gurobi_model


def convert_cvxpy_to_gurobi(expr, gurobi_vars):
    """
    Convert a cvxpy expression to a gurobi expression.

    Parameters:
    expr (cp.Expression): The cvxpy expression to convert.
    gurobi_vars (dict): A dictionary mapping cvxpy variables to gurobi variables.

    Returns:
    gp.LinExpr or gp.QuadExpr: The corresponding gurobi expression.
    """
    expr_type = expr.__class__.__name__

    if expr_type == 'Variable':
        return gurobi_vars[expr]
    elif expr_type == 'Constant':
        return expr.value
    elif expr_type == 'NegExpression':
        return -convert_cvxpy_to_gurobi(expr.args[0], gurobi_vars)
    elif expr_type == 'multiply':
        left, right = expr.args
        if isinstance(left, cp.Constant):
            return left.value * convert_cvxpy_to_gurobi(right, gurobi_vars)
        elif isinstance(right, cp.Constant):
            return right.value * convert_cvxpy_to_gurobi(left, gurobi_vars)
        else:
            raise NotImplementedError("Non-constant multiplication is not supported")
    elif expr_type == 'AddExpression':
        return gp.quicksum(convert_cvxpy_to_gurobi(arg, gurobi_vars) for arg in expr.args)
    elif expr_type == 'SubExpression':
        left, right = expr.args
        return convert_cvxpy_to_gurobi(left, gurobi_vars) - convert_cvxpy_to_gurobi(right, gurobi_vars)
    elif expr_type == 'DivExpression':
        left, right = expr.args
        if isinstance(right, cp.Constant):
            return convert_cvxpy_to_gurobi(left, gurobi_vars) / right.value
        else:
            raise NotImplementedError("Non-constant division is not supported")
    elif expr_type == 'Sum':
        return gp.quicksum(convert_cvxpy_to_gurobi(arg, gurobi_vars) for arg in expr.args[0])
    elif expr_type == 'Index':
        var = expr.args[0]
        idx = expr.get_data()[1]
        return gurobi_vars[var.name()+f'[{idx}]']
    else:
