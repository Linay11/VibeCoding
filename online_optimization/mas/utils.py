...
def sample_around_points(b, r,
    ...
    """
    np.random.seed(0)
    column_names = [f'b{i}' for i in range(len(b))]  # Generate column names like b0, b1, ..., b9
    df_samples = pd.DataFrame(columns=column_names)
    num_b = len(b)
    for i in tqdm(range(n_total)):
        dn = np.random.randn(num_b + 2, 1)
        norms_d = np.linalg.norm(dn)
        dn_normalized = dn / norms_d
        b_new = (dn_normalized[:num_b].flatten() * r) + b
...
file = filedir + '/mas76.mps'
get_instance(file, 1000, r=2, ptype=ptype)
