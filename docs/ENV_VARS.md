...
|---|---|---|---|---|---|
| `VITE_API_BASE_URL` | Frontend (Vite) | API base URL prepended to frontend requests | `http://127.0.0.1:8000` in frontend code when env is missing | `VITE_API_BASE_URL=http://127.0.0.1:8000` | Local frontend + SSH tunnel to remote backend |
| `VITE_API_BASE` | Frontend (Vite, legacy) | Backward-compatible fallback key for API base URL | unset | `VITE_API_BASE=http://127.0.0.1:8000` | Legacy local setups before migration |
...
| `POWER118_ML_MODEL_PATH` | Backend adapter / scripts | Override path for the power-118 ML model artifact | `backend_adapter/data/power118_ml_model.joblib` | `POWER118_ML_MODEL_PATH=/data/vibecoding/power118_ml_model.joblib` | Remote model artifact storage |
| `POWER118_ML_METADATA_PATH` | Backend adapter / scripts | Override path for the power-118 ML metadata JSON | `backend_adapter/data/power118_ml_metadata.json` | `POWER118_ML_METADATA_PATH=/data/vibecoding/power118_ml_metadata.json` | Remote metadata storage |
...
- Frontend:
  - `frontend/src/services/optimizerApi.js` reads `import.meta.env.VITE_API_BASE_URL`.
  - It also accepts legacy `import.meta.env.VITE_API_BASE` as fallback.
...
- Backend adapter:
  ...
  - `backend_adapter/services/power118_ml_model.py` reads:
    - `POWER118_ML_MODEL_PATH`
    - `POWER118_ML_METADATA_PATH`
- Script layer:
...
### 4) Override power-118 model artifact paths

```bash
POWER118_ML_MODEL_PATH=/data/vibecoding/power118_ml_model.joblib \
POWER118_ML_METADATA_PATH=/data/vibecoding/power118_ml_metadata.json \
...
- For current dev mode, keep `VITE_API_BASE_URL` pointed to local tunnel endpoint (`127.0.0.1:8000`), not directly to remote AutoDL public IP.
- Avoid `BACKEND_ADAPTER_CORS=*` in stricter environments; use explicit origins.
- `POWER118_ML_MODEL_PATH` and `POWER118_ML_METADATA_PATH` should point to artifacts generated by `python scripts/train_power118_model.py`.
