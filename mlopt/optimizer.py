...
from mlopt.cluster_filter import Cluster_Filter, strategy_degeneration
...
class Optimizer(object):
    ...
    def load_training_data(self, file_name):
        ...
        self.result = data_dict['result'] if 'result' in data_dict else None
    ...
    def get_samples(self, X=None, sampling_fn=None,
        ...
        if X is not None:
            ...
            feasible_indices = [i for i, x in enumerate(tqdm(results)) if not np.isnan(x['x']).any()]
            results = [results[i] for i in feasible_indices]
            X_filtered = X.iloc[feasible_indices]
            ...
            if filter_strategies:
                ...
                self.cluster_filter(parallel=parallel)
    ...
    def cluster_filter(self, parallel=True):
        ...
        self._filter = Cluster_Filter(X_train=self.X_train,
                                      y_train=self.y_train,
                                      obj_train=self.obj_train,
                                      encoding=self.encoding,
                                      problem=self._problem)
        self.y_train, self.encoding, self.X_train, self.obj_train, self.result = \
    ...
    def cal_res(self, parallel=True, batch_size=stg.JOBLIB_BATCH_SIZE):
        if self.X_train is None:
            e.value_error("Not enough arguments to train the model")

        n_jobs = u.get_n_processes() if parallel else 1
        self.result = np.array([None] * len(self.X_train))
        degeneration_results = Parallel(n_jobs=n_jobs, batch_size=batch_size)(
            delayed(strategy_degeneration)(self.X_train.iloc[i], self.obj_train[i],
                                           self.encoding, self._problem, self.result[i])
            for i in tqdm(range(len(self.X_train)), desc="CALCULATING RESULTS")
        )
        degeneration_matrix = [result[0] for result in degeneration_results]
        result_matrix = [result[1] for result in degeneration_results]
        result_matrix = np.array(result_matrix)
        self.result = result_matrix
        zero_rows = np.all(degeneration_matrix == 0, axis=1)
        has_zero_row = np.any(zero_rows)
        # 根据是否有全零行执行不同的代码
        if has_zero_row:
            print("矩阵中有全零行。执行相关操作...")
            # 使用 numpy.any() 沿着列方向检查 (axis=1)，找出非全零行
            non_zero_row_mask = np.any(degeneration_matrix != 0, axis=1)
            # 选择这些行
            degeneration_matrix_01 = degeneration_matrix[non_zero_row_mask]
            self.result = result_matrix[non_zero_row_mask]
    ...
    def choose_best(self, problem_data, labels, parallel=True,
    ...
    def solve(self, X,
        ...
        if use_cache and not self._solver_cache:
            e.warning("Solver cache requested but the cache has "
                      ...
                      "Possibly parameters in problem matrices.")
    ...
    def from_file(cls, file_name):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            optimizer_dict = pd.read_pickle(optimizer_file_name)
