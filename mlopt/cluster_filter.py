...
def strategy_degeneration(theta, obj_train, encoding, problem, res):
    ...
    if res is None:
        results = [dict([(key, problem.solve(strategy=strategy)[key]) for key in ['cost', 'infeasibility', 'strategy']])
...
class Cluster_Filter(object):
    ...
    def __init__(self,
        ...
        self.result = result if result is not None else np.array([None] * num_samples)
    ...
    def degenaration_matrix_generation(self, batch_size, parallel=True):
        ...
        degeneration_results = Parallel(n_jobs=n_jobs, batch_size=batch_size)(
            delayed(strategy_degeneration)(self.X_train.iloc[i], self.obj_train[i],
                                           self.encoding, self.problem, self.result[i])
            for i in tqdm(range(len(self.X_train)))
        )
        ...
        return degeneration_matrix_01, result_matrix
    ...
    def filter(self,
        ...
        #获得所有样本与其适用的约简策略矩阵
        degeneration_matrix_01, result_matrix = self.degenaration_matrix_generation(batch_size=batch_size,
                                                                                    parallel=parallel)
        ...
        for k in range(max_iter):
            ...
            selected_strategies, label_set = self.select_strategies_setcover(
                degeneration_matrix_01)  #返回选择的策略，以及这些策略cover的实例组成的字典

            # Reassign encodings and labels
            self.encoding = [self.encoding[i] for i in selected_strategies]
            res_m = []
            for i in range(n_samples):
                res = [self.result[i][j] for j in selected_strategies]
                res_m.append(res)
            self.result = np.array(res_m)
        ...
        return self.y_train, self.encoding, self.X_train, self.obj_train, self.result
